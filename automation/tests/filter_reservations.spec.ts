...
test('test', async ({ page }) => {
  ...
  // Intentar detectar si ya hay registros visibles (por ejemplo filas de una tabla)
  const table = page.getByRole('table').first();
  const rows = table.getByRole('row');
  const rowCount = await rows.count();

  if (rowCount > 1) {
    // Ya hay al menos una reserva listada (ignorando cabecera), finalizamos el test
    await expect(rows.nth(1)).toBeVisible({ timeout: 10000 });
    return;
  }

  // ========== FILTROS AVANZADOS POR RANGO DE FECHAS ==========
  // Abrir panel de filtros usando el botón de "Filtrar lista"
  ...
  // Abrir selector de rango de fechas "Fecha de la reserva"
  ...
  // Seleccionar fecha de inicio 01/01/2026
  ...
  // Seleccionar fecha fin 25/02/2026
